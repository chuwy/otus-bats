...
object Show {
  ...
  implicit val intShow: Show[Int] = fromFunction[Int](_.toString)
  implicit val strShow: Show[String] = fromFunction[String](str => str)
  implicit val boolShow: Show[Boolean] = fromFunction[Boolean](_.toString)


  // 1.2 Instances with conditional implicit

  implicit def listShow[A](implicit ev: Show[A]): Show[List[A]] = (l: List[A]) => l.map(ev.show(_)).mkString
  ...
  def mkString_[A: Show](list: List[A], begin: String, end: String, separator: String): String =
    list.show.mkString(begin, end, separator)
